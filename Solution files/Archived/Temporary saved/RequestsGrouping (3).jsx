...
const saveFiltersToBackend = useCallback(async (updatedFilters, updatedRequests) => {
  ...
  try {
    ...
    const response = await axios.post(`http://localhost:8000/api/temp-lists/save_list/`, {
      ...
      filter_params: filtersWithExpiration.filter_params, // ✅ Передаємо фільтри окремо
      expires_at: filtersWithExpiration.expires_at,
      requests: formattedRequests // ✅ Передаємо заявки окремо
    }, {
      headers: { Authorization: `Bearer ${token}` }
    });
