...
function RequestsGrouping({ onCheckboxClick,
  onUpdateRightTable,
  selectedRequestIds = [],
  onSelectedRequestsChange = () => {}}) {
    ...
    useEffect(() => {
      if (pendingSelections.length > 0 || selectedRequests.length === 0) {
        updateRequestsInStorage(pendingSelections);
        onUpdateRightTable(); // ✅ синхронізація правої таблиці
      }
    }, [pendingSelections]);
    useEffect(() => {
      // 1. Отримуємо вибрані заявки з sessionStorage
      const storedFilters = JSON.parse(sessionStorage.getItem("filters")) || {};
      const selectedIds = (storedFilters.requests || []).map((r) => r.id);
    
      // 2. Перевіряємо, чи вже завантажені заявки в лівій таблиці
      if (!passengerRequests || !Array.isArray(passengerRequests.left)) {
        console.warn("⚠️ passengerRequests.left ще не готовий");
        return;
      }
    
      // 3. Фільтруємо тільки ті заявки, яких немає серед обраних
      const filteredLeftRequests = passengerRequests.left.filter(
        (req) => !selectedIds.includes(req.id)
      );
    
      // 4. Оновлюємо стан лівої таблиці
      setPassengerRequests((prev) => ({
        ...prev,
        left: filteredLeftRequests,
      }));
    ...
    useEffect(() => {
      const storedFilters = JSON.parse(sessionStorage.getItem("filters")) || {};
      const storedRequests = storedFilters.requests || [];
    
      // якщо selectedRequests вже збігаються — не оновлюємо
      const same = selectedRequests.length === storedRequests.length &&
        selectedRequests.every((r, i) => r.id === storedRequests[i]?.id);
    
      if (!same) {
        console.log("🔁 Синхронізуємо selectedRequests із sessionStorage");
        setSelectedRequests(storedRequests);
      }
    }, [filters]);
...
const selectedIds = (JSON.parse(sessionStorage.getItem("filters"))?.requests || []).map(r => r.id);

const filteredRequests = passengerRequests
  .filter(req => !selectedIds.includes(req.id)) // ⛔ прибрати вже відібрані
  .filter(request =>
...
const handleAddToListChange = (request) => {
  ...
  const updatedSelections = isSelected
    ? selectedRequestIds.filter((id) => id !== request.id).map((id) =>
        ({ id })) // мінімальна форма, або бери повну, якщо є
    : [...selectedRequestIds, request];

  onSelectedRequestsChange(updatedSelections);
  updateRequestsInStorage(updatedSelections);
  onUpdateRightTable(); // праву таблицю оновлює
};
...
const columnDefs = [
    { headerName: t("request_id"), field: "id", width: 60 },
    {
      ...
      width: 70,
      cellRenderer: (params) => {
        const isChecked = selectedRequestIds.includes(params.data.id);
        return (
          <input
            type="checkbox"
            checked={isChecked}
