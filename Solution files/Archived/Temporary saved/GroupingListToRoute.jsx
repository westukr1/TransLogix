...
const fetchFilters = async () => {
  console.log("📤 Запит на отримання активного списку фільтрів...");
    try {
        ...
        if (response.data) {
          console.log("✅ Отримано фільтри:", response.data);
            setFilters(response.data.filter_params);
            fetchRequests(response.data.filter_params);
        } else {
            console.log("❌ Фільтри не знайдені, створюємо дефолтні:");
            setDefaultFilters();
        }
    } catch (error) {
...
const setDefaultFilters = async () => {
  const defaultFilters = {
      ...
      allowLongInterval: false,
      includeWorkAndHome: false,
      direction: 'home',
      showIncluded: false,
      showInRoute: false
  };
  console.log("📤 Відправка дефолтних фільтрів на сервер:", defaultFilters);
  setFilters(defaultFilters);
  await saveFilters(defaultFilters);
  fetchRequests(defaultFilters);
};
...
// тимчасово закосітив цю функцію, щоб спробувати аналогічну
  const fetchRequests = () => {
    ...
    console.log("📤 Запит на отримання списку заявок з фільтрами:", filters);
    let storedRequests = [];
    try {
   ...
   const updateSelectedRequests = (requestId) => {
    ...
    fetchRequests(); // Оновлюємо таблиці після зміни
  };
  ...
  const fetchPassengerLists = async () => {
    try {
      const formattedStartDate = filters.start_date && dayjs(filters.start_date).isValid()
        ? dayjs(filters.start_date).format("YYYY-MM-DDTHH:mm:ss")
        : null;

      const formattedEndDate = filters.end_date && dayjs(filters.end_date).isValid()
      ...
      const response = await axios.get(
        ...
        {
          params: {
            estimated_start_time__gte: formattedStartDate || undefined,
            estimated_end_time__lte: formattedEndDate || undefined,
...
useEffect(() => {
  try {
    ...
    if (parsedRequestIds.length > 0) {
      console.log("📌 Викликаємо fetchRequests() із фільтром:", parsedRequestIds);
      fetchRequests(parsedRequestIds);
    }
