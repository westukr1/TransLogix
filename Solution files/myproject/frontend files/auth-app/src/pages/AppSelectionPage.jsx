    ...
    <div className="asp-app-selection-page">
      ...
      <div className="asp-right-column">
        <div className="asp-buttons-container">
          <button 
            className={`asp-auth-button ${allowedApps.operator_ui ? 'enabled' : 'disabled'}`}
            onClick={() => allowedApps.operator_ui && handleNavigation('/operator-ui')}
            disabled={!allowedApps.operator_ui}>
            {t('logistic_operator')}
          </button>
            
          <button 
            className={`asp-auth-button ${allowedApps.finance_manager ? 'enabled' : 'disabled'}`}
            onClick={() => allowedApps.finance_manager && handleNavigation('/finance-manager')}
            disabled={!allowedApps.finance_manager}>
            {t('finance_manager')}
          </button>
            
          <button 
            className={`asp-auth-button ${allowedApps.admin ? 'enabled' : 'disabled'}`}
          ...
          <button className="asp-auth-button" onClick={() => navigate('/reset-password')}>
             {t('reset_password')}
          </button>
          <button
