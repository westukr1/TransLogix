...
const PassengerForm = ({ onClose, onSave }) => {
    ...
    const [passengerData, setPassengerData] = useState({
      ...
      workAddresses: [{ country: 'Україна', region: '', city: '', district: '', street: '', house_number: '', latitude: '', longitude: '', point_type: 'work' }], // Новий тип "Робота"
      phoneNumber: '',
      email: '',
    });
...
const PassengersTableView = () => {
  const navigate = useNavigate();
   ...
   // Отримуємо мову з LocalStorage під час завантаження компонента
    ...
    const [endDate, setEndDate] = useState(new Date(new Date().setDate(new Date().getDate() + 15)));
    ...
    const confirmation = window.confirm(
      ...
      try {
          ...
          const response = await fetch(`http://localhost:8000/api/passengers/${passengerId}/toggle-active/`, {
              method: 'POST',
              headers: {
          ...
          setPassengersRowData(prevData =>
              prevData.map((row) => row.id === passengerId ? { ...row, is_active: currentValue } : row)
          );
      }
  };
    ...
    const handleCheckboxChange = async (passengerId, currentValue) => {
      try {
          ...
          const token = localStorage.getItem('access_token');
          const response = await fetch(`http://localhost:8000/api/passengers/${passengerId}/toggle-select/`, {
              method: 'POST',
              headers: {
                  'Authorization': `Bearer ${token}`,
                  'Content-Type': 'application/json',
          ...
          setPassengersRowData((prevData) =>
              prevData.map((row) =>
                  row.id === passengerId ? { ...row, is_selected: data.is_selected } : row
              )
          );
    ...
    const handleSelectAll = async (checked) => {
      ...
      try {
          ...
          const response = await fetch('http://localhost:8000/api/passengers/toggle-select-all/', {
              method: 'POST',
              headers: {
                  'Authorization': `Bearer ${token}`,
                  'Content-Type': 'application/json',
          ...
          console.error('Error updating selection status for all passengers:', error);
    ...
    const formatAddress = (passenger) => {
      if (passenger) {
        const { country, region, district, city, street, house_number } = passenger;
        const formattedAddress = [street, house_number, city, region, district, country].filter(Boolean).join(', ');
    ...
    const filterBySelectedPassengers = () => {
      const selectedPassengerIds = passengersRowData
        ...
        setRoutesRowData(routesRowData.filter(route =>
            selectedPassengerIds.includes(route.passenger_id)
        ));
        setLocationsRowData(locationsRowData.filter(location =>
    ...
    const handleRowDoubleClick = (passenger) => {
      ...
      navigate(`/edit-passenger/${passenger.id}`, { state: { passengerData: passenger } });
  };
  ...
  const handleToggleCoordinateActive = async (coordinatePointId, currentValue) => {
    const confirmation = window.confirm(
      ...
      // Якщо користувач натиснув "Cancel", зупиняємо функцію
      if (!confirmation) return;
    console.log("Toggling active status for coordinatePointId:", coordinatePointId); // Add this line
  ...
  const handleAddressRowDoubleClick = (address) => {
    ...
    console.log("Navigating with passengerId:", passengerId, "and coordinatePointId:", coordinatePointId);
    navigate(`/edit-passenger-addresses`, { state: { passengerId, coordinatePointId } });
  };
    ...
    const [passengersColumnDefs] = useState([
      {
        ...
        cellRenderer: (params) => (
          <input
              ...
              onChange={() => handleCheckboxChange(params.data.id, params.data.is_selected)}
            />
          ),
        },
        ...
        { field: 'pickup_address.city', headerName: t('city'), valueGetter: (params) => params.data.city || '' },
        { field: 'pickup_address.street', headerName: t('street'), valueGetter: (params) => params.data.street || '' },
        { field: 'pickup_address.house_number', headerName: t('house_number'), valueGetter: (params) => params.data.house_number || '' },
        
        { field: 'pickup_address.district', headerName: t('district'), valueGetter: (params) => params.data.district || '' },
        { field: 'pickup_address.region', headerName: t('region'), valueGetter: (params) => params.data.region || '' },
        { field: 'pickup_address.country', headerName: t('country'), valueGetter: (params) => params.data.country || '' },
        { field: 'pickup_address.latitude', headerName: t('latitude'), valueGetter: (params) => params.data.latitude || '' },
        { field: 'pickup_address.longitude', headerName: t('longitude'), valueGetter: (params) => params.data.longitude || '' },

        { field: 'email', headerName: t('email') },
        { 
          field: 'id', 
          headerName: 'ID', 
          cellStyle: { display: 'flex', justifyContent: 'center', alignItems: 'center' } // Вирівнювання по центру
        },
        {
          field: 'is_active',
          headerName: t('active'),
          cellRenderer: (params) => (
            <ToggleSwitch
              ...
              onChange={() => handleToggleActive(params.data.id, params.data.is_active)}
            />
          ),
          width: 100,
        }
    ...
    const [locationsColumnDefs] = useState([
      ...
      { field: 'latitude', headerName: t('latitude'), 
        cellStyle: { display: 'flex', justifyContent: 'center', alignItems: 'center' } // Вирівнювання по центру
      },
      { field: 'longitude', headerName: t('longitude'), 
        cellStyle: { display: 'flex', justifyContent: 'center', alignItems: 'center' } // Вирівнювання по центру
      },
      ...
      { field: 'house_number', headerName: t('house_number'),
        cellStyle: { display: 'flex', justifyContent: 'center', alignItems: 'center' } // Вирівнювання по центру
      },
      ...
      { field: 'owner_id', headerName: t('owner_id'), cellStyle: { display: 'flex', justifyContent: 'center', alignItems: 'center' } },
    ...
    const fetchLocations = async () => {
        const token = localStorage.getItem('access_token');
        const response = await fetch('http://localhost:8000/api/filtered-coordinates/', {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        const data = await response.json();
        ...
        <div className="passenger-table-view">
          ...
          <div className="passenger-table-content"style={{ height: 1000 }}>
            ...
            <div className="passenger-left-column">
              ...
              <Link to="/create-passenger" 
              className="ptv-add-passenger-button"
              >{t('add_passenger')}</Link> {/* Виправлено для навігації */}
              
            </div>
    
            <div className="passenger-right-column" style={{ height: 1000, width: '90%', overflowX: 'auto' }}>
              {/* Відображаємо форму або таблиці залежно від стану */}
              {showForm ? (
                <PassengerForm onClose={() => setShowForm(false)} onSave={handleSavePassenger} />
              ) : (
                <>
                  <div className="passenger-upper-right">
                    ...
                    <div className="ag-theme-alpine" style={{ height: '30%', width: '100%', overflowX: 'auto' }}>
                      <AgGridReact
                        headerHeight={20} // Висота заголовків
                        rowHeight={20}
                        rowData={passengersRowData}
                        columnDefs={passengersColumnDefs}
                        onRowDoubleClicked={(params) => handleRowDoubleClick(params.data)}
                        defaultColDef={{
                          sortable: true,
                          filter: true,
                          flex: 1,
                  ...
                  <div className="passenger-upper-right">
                    ...
                    <div className="ag-theme-alpine" style={{ height: '30%', width: '100%', overflowX: 'auto' }}>
                      <AgGridReact
                        ...
                        onRowDoubleClicked={(params) => handleAddressRowDoubleClick(params.data)}
